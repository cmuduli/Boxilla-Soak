...
public class DkmPresets5 extends StartupTestCase {

	DkmMethods methods = new DkmMethods();
	final static Logger log = Logger.getLogger(DkmPresets5.class);
	...
	public void dependencySwitchAdd() throws InterruptedException {
		log.info("Test Preparation  - DKM - Add Switch");
		methods.navigateToDkmSwitches(driver);
		methods.addDkmSwitch(driver, "10.211.128.123", "Test_Switch");		//145 is alex switch
		log.info("DKM - Switch Added - Test Preparation Completed");
	}

	@Test(dependsOnMethods = { "dependencySwitchAdd" })
	public void dkmPreset5() throws InterruptedException {
		log.info("Partial preset testcase - Soak Test dkmPreset5 started");
		methods.resetAllDkm(driver);
		log.info("Starting dkmPreset5..");
		methods.navigateToDkmViewer(driver);
		methods.addSource(driver, "INVISAPC_TX14");
		methods.addDestination(driver, "INVISAPC_TX14", "DKM_RX3");
		methods.assertUI(driver, "INVISAPC_TX14");
		DkmElements.managePresetsBtn(driver).click();
		methods.timer(driver);
		DkmElements.createCustomPreset(driver).click();
		methods.timer(driver);
		DkmElements.presetSourceCheckbox(driver, 4).click();
		log.info("Preset source VTX" + 4 + " Selected");
		...
		methods.timer(driver);
		DkmElements.presetDestinationCheckbox(driver, 1, 1).click();
		...
		log.info("Preset button clicked..");
		methods.timer(driver);
		...
		methods.timer(driver);
		// Assert Connection
		Assert.assertTrue(DkmElements.activeConnectionCoutner(driver).getText().equalsIgnoreCase("1"),
				"Active connections is greater than 1");
		Assert.assertTrue(DkmElements.dkmConnectionRow(driver, "INVISAPC_TX188").getText().contains("DKM_RX3"),
				"INVISAPC_TX188 is not connected to DKM_RX3");
		methods.timer(driver);
		log.info("Partial preset testcase - Soak Test dkmPreset5 Completed");
